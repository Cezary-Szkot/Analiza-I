...
%    \hfill \texttt{WERSJA I}\\ \smallskip
%Niech funkcje rzeczywiste $f,g$ będą określone na przedziale $(a,b]$ i
%\begin{equation*}
%    \left( \lim_{x\to a^+}f(x)=0\land \lim_{x\to a^+}g(x)=0\right)\ \lor\ \left( \lim_{x\to a^+}f(x)=\pm\infty\land \lim_{x\to a^+}g(x)=\pm\infty\right)
%\end{equation*}
%\end{twier}
