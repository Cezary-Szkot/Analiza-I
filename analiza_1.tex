...
\begin{enumerate}[label=\textbf{(\arabic*)}]
    ...
    \end{equation*}
    \item \textit{metryka euklidesowa}, $\quad d_2:\R^n\times\R^n\ar\R$
    \begin{equation*}
        \forall_{x,y\in\R^n}\hquad \sqrt{\sum_{k=1}^{n}{(x_k-y_k)^2}}
    \end{equation*}
    \item \textit{uogólniona metryka euklidesowa}\footnote{Nie mogłem odnaleźć nazwy tej metryki, więc koniec końców
    tak ją nazwałem. Warto zatem nie przywiązywać do niej większej wagi.}, $\quad d_p:\R^n\times\R^n\ar\R$
    \begin{equation*}
...
Pary $(X,d_d)$, $(\R^n,d_1)$, ($\R^n,d_2$), ($\R^n,d_p$), ($\R^n,d_{\infty}$), ($\mathcal{B}(X,Y),d_{sup}$) są przestrzeniami
metrycznymi.
\medskip
...
\begin{defr}{Definicja 6.10: Domknięcie}
...
Domknięcie i wnętrze zbioru $A$ alternatywnie oznaczamy jako $cl\ A$ oraz $int\ A$.
