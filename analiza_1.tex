...
\forall_{\varepsilon>0}\ \exists_{\delta>0}\ \forall_{x\in A}\hquad&d_X(p,x)<\delta\implies d_Y(f(x),g)<\varepsilon,\\
...
\forall_{\varepsilon>0}\ \exists_{\delta>0}\ \forall_{x\in A\setminus\set{p}}\hquad&d_X(p,x)<\delta\implies d_Y(f(x),g)<\varepsilon,\\
                &x\in B_X(p,\delta)\implies f(x)\in B_Y(g,\varepsilon),
\end{aligned}$
    \item[$(H)$] $\forall_{\seq{x}\,\subset\,A}\hquad x_n\arr{d_X}p\implies f(x_n)\arr{d_Y}g$,
...
jest podzbiorem $X$. Ogwiazdkowane wersje definicji są spoza wykładu i dodałem je dlatego, że częściej się je na ogół spotyka, niż
zaprezentowane na zajęciach warianty.
...
\begin{defr}{Zbieżność od dołu/góry}
...
w punkcie $a\in\closure{A}$ \textbf{od dołu (od góry)}, jeżeli
    \begin{enumerate}
        \item[(C)] $\begin{aligned}[t]
        \forall_{\varepsilon>0}\ \exists_{\delta>0}\ \forall_{x\in B_X(a, \delta)\cap A}\hquad& f(x)\in(g-\varepsilon,g),\\
        &\left(f(x)\in(g,g+\varepsilon)\right),
        \end{aligned}$ 
        \item[(H)] $\begin{aligned}[t]
        \forall_{\seq{x}\subset X}\hquad x_n\arr{d_X}a\implies f(x_n)\arr{}g\ \land\ dddn\ \ &f(x_n)<g,\\
        &\left(f(x_n)>g\right).
        \end{aligned}$
    \end{enumerate}
\end{defr}

\begin{defr}{Granica niewłaściwa w punkcie}
...
$\boldsymbol{+\infty}$ \textbf{($\boldsymbol{-\infty}$) w punkcie} $a\in\closure{A}$, jeśli
    \begin{enumerate}
        \item[(C)] $\begin{aligned}[t]
        \forall_{r\in\R}\ \exists_{\delta>0}\ \forall_{x\in B_X(a, \delta)\cap A}\hquad &f(x)>r,\\
        &\left(f(x)<r\right),
        \end{aligned}$
        \item[(H)] $\forall_{\seq{x}\subset A}\hquad x_n\arr{d_X}a\implies f(x_n)\arr{}+\infty\ (-\infty)$.
    \end{enumerate}
\end{defr}
